...
def test_will_generate_global_variables_report_when_add_global_variables(metadata: Datadoc):
    state.metadata = metadata
    first_var_short_name = metadata.variables[0].short_name
    variable = state.metadata.variables_lookup.get(first_var_short_name)
    assert variable is not None
    ...
    assert report["multiplication_factor"]["display_value"] == 2
    assert report["multiplication_factor"]["num_vars"] == len(report["multiplication_factor"]["vars_updated"])
